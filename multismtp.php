...
Plugin URI: http://rxa.li/mymail?utm_campaign=wporg&utm_source=Multi+SMTP+for+MyMail
...
class MyMailMultiSMTP {
	...
	public function getnextserver( $use = null, $round = 0 ) {
		...
		if ( ! $mymail_multismtp_sent_within_period ) { $mymail_multismtp_sent_within_period = 0; }
	...
	public function save_sent_within_period( $value ) {
		...
		if ( $mymail_multismtp_sent_within_period ) { set_transient( '_mymail_send_period_' . $mymail_multismtp_current, $mymail_multismtp_sent_within_period ); }
	...
	public function metabox() {
		...
		$data = wp_parse_args( get_post_meta( $post->ID, 'mymail-multismtp', true ), array(
				'use_global' => true,
				'selected' => null,
		) );
...
?>
		...
		<table class="form-table">
			...
			<tr valign="top">
				...
				<td>
				<label><input type="radio" name="mymail_options[multismtp][<?php echo $i ?>][secure]" value="" <?php if ( ! $secure ) { echo ' checked'; } ?> class="smtp secure" data-port="25"> <?php _e( 'none', 'mymail-multismtp' ) ?></label>
				<label><input type="radio" name="mymail_options[multismtp][<?php echo $i ?>][secure]" value="ssl" <?php if ( $secure == 'ssl' ) { echo ' checked'; } ?> class="smtp secure" data-port="465"> SSL</label>
				<label><input type="radio" name="mymail_options[multismtp][<?php echo $i ?>][secure]" value="tls" <?php if ( $secure == 'tls' ) { echo ' checked'; } ?> class="smtp secure" data-port="465"> TLS</label>
				 </td>
			</tr>
			<tr valign="top">
...
?>
	<div id="message" class="error">
	  <p>
	   <strong>Multi SMTP for MyMail</strong> requires the <a href="http://rxa.li/mymail?utm_campaign=wporg&utm_source=Multi+SMTP+for+MyMail">MyMail Newsletter Plugin</a>, at least version <strong><?php echo MYMAIL_MULTISMTP_REQUIRED_VERSION?></strong>. Plugin deactivated.
	  </p>
	</div>
		<?php
	}
